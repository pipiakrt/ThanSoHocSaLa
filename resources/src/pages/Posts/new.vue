                    ...
                    <div class="card card-custom">
                        ...
                        <div class="card-body pt-0" id="loadTag">
                            <table class="table table-head-custom table-head-bg table-borderless table-vertical-center">
                                ...
                                <tbody>
                                    <template v-for="item in postnew">
                                        <tr :key="'row' + item.id">
                                            <td>
                        ...
                        <div>
                            <ValidationObserver ref="errors">
                                <div class="card-body">
                                    <div class="row mb-5">
                                        <div class="col-10">
                                            <input v-model="filterName" type="text" placeholder="Tên item." class="form-control form-control-sm form-filter datatable-input"/>
                                        </div>
                                        <div class="col-2">
...
export default {
    ...
    created() {
        ...
        axios('/api/postnews').then(res => {
            KTApp.unblock('#loadTag');
            this.postnew = res.data.data
        })
        this.getApi()
    },
    methods: {
        ...
        async submit(id) {
            if (await this.errors()) {
                ...
                axios.post('/api/postnews', params).then((res) => {
                    KTApp.unblockPage();
                    if (res.status == 201) {
                        params.id = res.data._id
                        params = this.posts.data.find(item => item.id == params.post_id)
                        this.postnew.push(params)
                        this.$refs['errors'].reset();
                        toastr.success("Tạo item. thành công!")
        ...
        destroy(id) {
            axios.delete('/api/postnews/' + id).then(res => {
                Swal.fire(
                    "Thành Công!",
                    "item. đã bị xóa hoàn toàn.",
                    "success"
                )
                this.postnew = this.postnew.filter(item => item.id !== id)
            })
        },
    },
}
