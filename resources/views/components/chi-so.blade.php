...
$ynghia = [
    [
        "name" => "sứ mệnh",
        "url" => "/gioi-thieu/su-menh",
        "desc" => "Chỉ số này tiết lộ bạn sinh ra để làm gì và trở thành ai trong cuộc đời này. Đây là chỉ số quan trọng thứ 2 trong bản đồ thần số học của bạn và bạn cần xét đến khi lựa chọn nghề nghiệp.",
    ],
    [
        "name" => "đường đời",
        "url" => "/gioi-thieu/duong-doi",
        "desc" => "Chỉ số này hé lộ con đường mà bạn sẽ trải qua trong cuộc đời này. Nó cho bạn thấy bạn sẽ gặp phải những trải nghiệm như thế nào, và bạn học được gì sau những trải nghiệm đó. Đây là chỉ số quan trọng nhất khi bạn xem thần số học của mình. Nó cung cấp nhiều thông tin về con người bạn và cuộc đời mà bạn sẽ sống.",
    ],
    [
        "name" => "nhân cách",
        "url" => "#",
        "desc" => "Chỉ số này thể hiện cách bạn phản ứng với những gì xảy ra trong cuộc đời mình. Chúng ta không kiểm soát được những gì đến với mình nhưng chúng ta hoàn toàn làm chủ được cách phản ứng với nó nên con số này bạn hoàn toàn có thể phát huy ưu điểm và khắc phục nhược điểm của nó được.",
    ],
    [
        "name" => "tâm hồn",
        "url" => "#",
        "desc" => "Tâm hồn, sự khao khát từ sâu bên trong của mỗi người. Chỉ số này hé lộ bí mật sâu thẳm trong bạn mong muốn bạn trở thành con người như thế nào thì mới cảm thấy thỏa mãn và trọn vẹn. Nói cách khác, nếu bạn có những đặc điểm tích cực của chỉ số này, tâm hồn bạn sẽ cảm thấy hạnh phúc.",
    ],
    [
        "name" => "trưởng thành",
        "url" => "#",
        "desc" => "Chỉ số này tiết lộ sự trưởng thành và thành đạt của bạn sẽ ở trên phương diện thế nào sau này. Nó cho biết bạn có thể trông đợi điều gì trong nửa sau cuộc đời, bạn cần để tâm tới chỉ số này khi đặt mục tiêu hay đưa ra quyết định trong dài hạn.",
    ],
]
@endphp
...
<style>
        ...
        .fixfdaf .tooltip:hover {
            cursor: help;
        }
        .fixfdaf .tooltip-spacing {
            height: 20px;
            margin: 8px;
            position: relative;
            width: 20px;
        ...
        .fixfdaf .tooltip:hover ~ .popup-text {
            display: block;
        }
        .fixfdaf .popup-bg {
            opacity: 0;
        ...
        .fixfdaf .tooltip:hover ~ .popup-bg {
            opacity: 1;
            transition: opacity 240ms 120ms cubic-bezier(0.4, 0, 0.2, 1);
        }
        .fixfdaf .tooltip:hover ~ .popup-outline .popup-outline-left {
            stroke-dasharray: 426px 426px;
            transition: stroke-dasharray 300ms cubic-bezier(0.4, 0, 0.2, 1);
        }
        .fixfdaf .tooltip:hover ~ .popup-outline .popup-outline-right {
            stroke-dashoffset: 0;
            transition: stroke-dashoffset 300ms cubic-bezier(0.4, 0, 0.2, 1);
        }
...
@foreach ($ynghia as $key => $item)
    <div class="modal" id="exampleModal-{{ $key }}" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content fixfdaf">
                <div class="card">
                    ...
                    <div class="text">
                        ...
                        <div class="info">
                            <p class="mb-1">Đăng nhập để xem được chỉ số {{ $item["name"] }} cùng giải thích chi tiết</p>
                            <ul class="px-3">
                                <li>
...
<div class="block-1 text-center text-white d-none d-md-block">
                            ...
                            <div class="box-group">
                                <div class="box">
                                    <div class="title" style="text-transform: uppercase;">
                                        {{ $item["name"] }}
                                        <div class="desc"><span style="text-transform: lowercase;">Ý nghĩa chỉ số {{ $item["name"] }}</span></div>
                                    </div>
                                </div>
                            </div>
                            <div class="item-drop">
                                <div class="group-item">
                                    <div class="drop-title"><h3 style="text-transform: capitalize;">{{ $item["name"] }}</h3></div>
                                    <div class="hr"></div>
                                    <div class="drop-desc"><span>{{ $item["desc"] }}</span></div>
                                </div>
                            </div>
                        </div>
                    </a>
...
<div class="block-1 text-center text-white d-block d-md-none">
    <div class="container container-block-1 mh-1150 ">
        <div class="row justify-content-center">
            <div class="col-12">
                <h2 class="text-dark titleheader title-h1">Ý NGHĨA CÁC CHỈ SỐ</h2>
            ...
            <div class="col-md-6 col-xl-4 col-lg-6">
                <div id="chiso" class="owl-carousel owl-theme d-block d-md-none">
                    @foreach ($ynghia as $key => $item)
                        <div class="item">
                            <a href="{{ $item["url"] }}" data-bs-toggle="{{ $key >= 2 ? "modal" : "" }}" data-bs-target="#exampleModal-{{ $key }}">
                                <div class="item {{ $key == 0 ? "active" : "" }}" style="background-image: url('/img/vt/2.png');">
